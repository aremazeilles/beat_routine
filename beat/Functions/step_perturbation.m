...
platform_angle=cell2mat(platformdata(:,[9,10]));
imposed_angle=cell2mat(platformdata(:,22));
...
for d=1:length(dir)-1
  angle_platform=acos(cos(platform_angle(dir(d):dir(d+1),1)).*cos(platform_angle(dir(d):dir(d+1),2)))*57.3;  %%column 9 and 10 contain the angle of the platform in x and y direction
 OS_i(d)= max(angle_platform)-max(imposed_angle(dir(d):dir(d+1),1)); %%compute overshoot for each direction with respect the imposed perturbation saved in the column 22
